...
selectTopic(e){
  ...
  fetch(BASE_URL_INTEREST+"getauthortopicdetails/?"+this.state.selectVal1+"&"+this.state.selectVal2+"&"+this.state.selectYear+"&"+e.target.value)
    .then(response =>  response.json())
    .then(json => {
        var series=[]  
        console.log("auth",json.authortopics[0])
        for(let i=0;i<json.authortopics[0].length;i++){
            series=series.concat([{name:json.authortopics[0][i],data:json.authortopics[1][i]}])
        ...
        this.setState({
          ...
          options:{
            colors : ['#00FFFF', '#7FFFD4','#FFE4C4', '#0000FF',
            '#D2691E', '#00008B', '#A9A9A9','#BDB76B','#FF8C00',
            '#9400D3','#FF00FF','#FFD700','#DC143C','#FFDAB9','#D2B48C',
            '#B0C4DE','#000000','#00FA9A','#DAA520','#ADFF2F'
          ],
            chart: {
                type: 'bar',
                height: 300,
...
componentWillMount(){
  ...
  fetch(BASE_URL_INTEREST+"getauthortopicdetails/?A. A. Economides&A. Ammari&all years&topic")
    .then(response =>  response.json())
    .then(json => {
        var series=[]  
        console.log("auth",json.authortopics[0])
...
selectyearValue(e){
  ...
  render() {
    var {isLoaded,selectYear,authors1,authors2,selectVal1,commontpcs,selectVal2,
      display,loader,display1,active1,active2}=this.state;
    ...
    var url="https://www.semanticscholar.org/search?author="+selectVal1+"&author="+selectVal2+"&q="
    ...
    return (
      <div className="App" style={{'height':'1000px','width':'700px'}}>
        ...
        <p>This visualization displays the comparison of topics/keywords between two researchers</p>
        <br></br>
        {/* <Select  
...
<div style={{width: '315px'}}>            
        ...
        <br></br>
              ...
              <Button outline value="keyword" active={active2} color="primary" onClick={this.selectKey}>Keyword</Button>  
       ...
       ></i> 
       {
        this.state.imageTooltipOpen && (
                         ...
                         <br></br>
              <br></br>
              <div style={{'backgroundColor':'white','display':display1,'position':'absolute','width':'100px','marginLeft':'300px','marginTop':'100px'}}>
              <Loader type="Bars" visible={loader} color="#00BFFF" height={100} width={100}></Loader>
              </div>
<div style={{'display':display}}>
              <h5>Common topics/keywords</h5>
              {commontpcs.length==0?(<div>No common topics/keywords found</div>):(
              commontpcs.map((number) =>
              <Badge href={url+number} target="_blank" color="info" style={{'marginLeft':'10px'}}>{number}</Badge>))}
       ...
       <div style={{'marginLeft':'30px'}}>
        ...
        <Label>Select two researchers to compare</Label>
           ...
           <br></br>
              ...
              <Button color="primary" value="topic" onClick={this.selectTopic}>Topic</Button>
              <Button color="primary" value="keyword" onClick={this.selectKey}>Keyword</Button> 
              <i  className="fas fa-question-circle text-blue"
