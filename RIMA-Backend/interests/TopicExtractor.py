...
def fetchAllTopics(yearList,algorithm):
    ...
    for val in yearList:
        ...
        try:
            ...
            postgreSQL_insert_Query = "insert into Topics(year,topics,algorithm,keywords) values('"+str(val)+"','"+str(wikis_noquotes)+"','"+str(algorithm)+"','"+str(keywords_noquotes)+"')"
...
def fetchTopics(algorithm):
    ...
    try:
        ...
        cursor=conn.cursor()
        postgreSQL_insert_Query = "insert into Topics(year,topics,algorithm,keywords) values('"+str("all years")+"','"+str(wikis_noquotes)+"','"+str(algorithm)+"','"+str(keywords_noquotes)+"')"
        cursor.execute(postgreSQL_insert_Query)
        conn.commit()
...
def fetchData_authorTable():
    ...
    for val in authors_list:
         author_data = requests.get(f"https://api.semanticscholar.org/v1/author/{val}").json()
...
def fetchAbstracts_author():
    ...
    try:
        ...
        for val in author_list:
            ...
            lak_records = cursor.fetchall()
            data_lak = pd.read_sql(postgreSQL_select_Query, conn)
            data_lak['titleAndabstract']=data_lak['title']+" "+data_lak['abstract']
            ...
            if "'" in string_abstract:
                ...
                string_abstract= string_abstract[ : index] + "'" + string_abstract[index : ] 
            val_id_data=requests.get(f"https://api.semanticscholar.org/v1/author/{val}").json()
            #print(val_id_data)
            ...
            postgreSQL_insert_Query = "insert into Authors1(authorID ,name ,titlenabs,conference) values('"+str(val)+"','"+str(val_id)+"','"+str(data_lak['titleAndabstract'].values[i])+"','"+str(data_lak['year'].values[i])+"','"+str(conference)+"')"
            cursor.execute(postgreSQL_insert_Query)
            conn.commit()
