(function(undefined) {
  ...
  // Initialize package:
  window.sigma.utils.pkg("sigma.canvas.nodes");
  window.sigma.utils.pkg("sigma.svg.nodes");

  var sigInst = undefined;
  var imgCache = {};

  var initPlugin = function(inst) {
    let sig = new window.sigma();
    sig.refresh();
    sigInst = sig;
  };

  var drawImage = function(node, x, y, size, context) {
    // console.log(node, x, y, size, context);
  ...
  var register = function(name, drawShape, drawBorder) {
    window.sigma.canvas.nodes[name] = function(node, context, settings) {
      var args = arguments,
  ...
  window.ShapeLibrary.enumerate().forEach(function(shape) {
