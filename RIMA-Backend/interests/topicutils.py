...
def getTopics(year,query):
    try:
        ...
        url = up.urlparse("postgres://yrrhzsue:E7FQJw6yw3mha6BvbimQJVNWGRR7Lwn6@ruby.db.elephantsql.com:5432/yrrhzsue")
...
def applyTopicMiningKeyword(year,number):
    ...
    sorted_topics_tuple = sorted(topics_list_ref.items() , reverse=True, key=operator.itemgetter(1))
    print(sorted_topics_tuple)
    list_topics_y1 = [{"text":a[0],"value":a[1]} for a in sorted_topics_tuple]
  
    newlist = [{"text": key, "value": val} for key, val in topics_list_ref.items()]
...
def getTopKeywords(year):
    query="select t.keywords,t.year,l.abstract,l.title from Topics t join LAKData l on l.year=t.year where t.year='"+str(year)+"'"
    lak_data=getTopics(year,query)
    lak_data["titleandabstract"]=lak_data["title"]+" "+lak_data["abstract"]
    topics_list=lak_data['keywords'].values[0]
    topics_list=topics_list.replace("{","")
    topics_list=topics_list.replace("}","")
    print("******************************************************************************")
...
def getTopTopics(year):
    print("Bar chart...")
    query="select t.topics,t.year,l.abstract,l.title from Topics t join LAKData l on l.year=t.year where t.year='"+str(year)+"'"
    lak_data=getTopics(year,query)
    lak_data["titleandabstract"]=lak_data["title"]+" "+lak_data["abstract"]
    topics_list=lak_data['topics'].values[0]
    topics_list=topics_list.replace("{","")
...
def getTopicDetails(topic,year):
    ...
    for key in list(lak_abstract['titleAndAbstract'].values):
        ...
        if topic.lower() in key.lower():
            ...
            alldetails_df=lak_abstract[lak_abstract["titleAndAbstract"]==key]
            list_titles.append(alldetails_df["title"].values[0])
            dict_vals={"title":alldetails_df["title"].values[0],"abstract":alldetails_df["abstract"].values[0]}              
            list_details.append(dict_vals)
            dict_freqs.update({key.lower().count(topic.lower()):alldetails_df["title"].values[0]})

                
    #dict_final={"label":list_details,"value":weights_list} 
    sorted_dict = dict(sorted(dict_freqs.items(), reverse=True,key=operator.itemgetter(0)))
   ...
   # list_dict_freqs={"title":list_titles,"frequency":list_tpcfreq}
    ...
    for val,weight in zip(list_details,weights_list):
        d={}
        d['label']=str(val).replace("{","").replace("}","").replace(',','\n',1)
        d['value']=weight
        list_final.append(d)
...
def getKeyDetails(keyword,year):
    ...
    for key in list(lak_abstract['titleAndAbstract'].values):
        ...
        if key.lower().find(keyword.lower())!=-1:
            ...
            dict_vals={"title":alldetails_df["title"].values[0],"abstract":alldetails_df["abstract"].values[0]}              
            list_details.append(dict_vals)
            dict_freqs.update({key.lower().count(keyword.lower()):alldetails_df["title"].values[0]})
...
def compareTopics(year1,year2):
    ...
    topics_list_ref_year2 = {i.split(': ')[0]: i.split(': ')[1] for i in topics_list_year2.split(', ')}
...
def getAllKeywordsAllYears():
    ...
    list_dict = [{"value": val, "label": val} for val in list(set(flatList_topics))]
    return list_dict
...
def getKeyWeightsAllYears(topic):
    ...
    topics_yearwise.newtopics = topics_yearwise.newtopics.apply(lambda x: list(x.items()))
    topics_yearwise = topics_yearwise.explode('newtopics')
    topics_yearwise[['topic', 'weight']] = pd.DataFrame(topics_yearwise.newtopics.tolist(),index=topics_yearwise.index)
...
def getDataForPieKeys(year,number):
    ...
    topics_yearwise=topics_yearwise.sort_values(by=['weight'],ascending=False)
    topics_yearwise[['weight']]=MinMaxScaler().fit_transform(topics_yearwise[['weight']])
    topics_yearwise['percent'] = (topics_yearwise['weight'] / topics_yearwise['weight'].sum()) * 100
...
def getMultipleYearKeyJourney(listoftopics):
    try:
        list_weights=[]
        list_years=[]
        lak_years=['2011','2012','2013','2014','2015','2016','2017','2018','2019','2020']
        ...
        for l1, l2 in zip(list_weights, list_years):
            l1 = iter(l1)
            sub_list = [next(l1) if str(index+2011) in l2 else '0' for index in range(10)]
...
def getAuthorFromAuthorName(author,topic,year):
    ...
    authors_mod = pd.DataFrame({'authors': np.hstack(lak_id['authors'].str.split(',')),
                       'authorids': np.hstack(lak_id['authorids'].str.split(','))})                   
    print(authors_mod)
    ...
    lak_count['titleAndAbstract']=lak_count['title']+" "+lak_count['abstract']
    ...
    l_authorids=[]
    count=0
    for val,id,authors,titles,authorids in zip(list(lak_count['titleAndAbstract'].values),list(lak_count['id'].values),list(lak_count['authors'].values),
    list(lak_count['title'].values),list(lak_count['authorids'].values)):
...
def getTopicEvoultion():
    ...
    df_sankey_small=topics_yearwise.groupby('year').apply(lambda x: x.ffill().bfill()).drop_duplicates()
    df_sankey_small=df_sankey_small.pivot(index='topic', columns='year', values='weight')
...
def generateVennDataKeys(year1,year2):
    ...
    sorted_topics_tuple2 = sorted(topics_list_ref_year2.items() , reverse=True, key=operator.itemgetter(1))
    ...
    list_intersect_y1y2=list(set(list_topics_y1).intersection(set(list_topics_y2)))
    ...
    v.get_label_by_id('10').set_text('\n'.join(list(set(list_topics_y1)-set(list_topics_y2))))
    label1 = v.get_label_by_id('10')          
    label1.set_fontsize(7) 
    v.get_label_by_id('01').set_text('\n'.join(list(set(list_topics_y2)-set(list_topics_y1))))
...
def generateVennData(year1,year2):
    ...
    ax.set_title('Common topics for the years '+str(year1)+' and '+str(year2), fontsize=12)
    v = venn2_unweighted(subsets = (40, 40, 25),
    set_labels=[str(year1), str(year2)])
    v.get_patch_by_id('10').set_alpha(0.3)
    v.get_patch_by_id('10').set_color('#86AD41')
...
def searchForTopics(year,keyword):
    try:
        ...
        lak_data["titleAndAbstract"]=lak_data["title"]+" "+lak_data["abstract"]
        list_titles=[]
        for val1,val2 in zip(list(lak_data["titleAndAbstract"].values),lak_data["title"]):
            if keyword in val1:
                val1.count(keyword)
                list_titles.append(val2)
        list_titles=list(set(list_titles))
        query ="select authors from LAKData where title in"+str(tuple(list_titles))+"and year='"+year+"'"
        ...
        result = []
        result = [{'source': start, 'target': end}
        ...
        if len(result)==0:
            dict_xy={"nodes":[{"id":"No Data Found","size":1000,"x":100,"y":200}]}
        else:
            dict_xy={"nodes":list_nodes,"links":l2}
    except UnboundLocalError:
        dict_xy={"nodes":[{"id":"No Data Found","size":500,"x":500,"y":200}]}
    except:
        dict_xy={"nodes":[{"id":"No Data Found","size":500,"x":500,"y":200}]}
...
def getTopKeysForAllYear(list_of_years):
    query="select keywords,year from Topics where year in"+str(tuple(list_of_years))
    lak_data=getTopics("",query)
    list_dicts=[]
...
def getFlowChartDataKeywords(year,searchword):
    ...
    keys_list=keys_list.replace("}","")
    print("******************************************************************************")
    keys_list_ref = {i.split(': ')[0]: i.split(': ')[1] for i in keys_list.split(', ')}
    ...
    print(keys_list_ref)
    sorted_keys_tuple = sorted(keys_list_ref.items() , reverse=True, key=operator.itemgetter(1))    
...
def getAbstractbasedonKeyword(year,keyword):
    ...
    LakAbstract["titleAndAbstract"]=LakAbstract["title"]+"\n"+LakAbstract["abstract"]
    dict_lists=[]
...
def getDataAuthorComparisonKeywords(year,author1,author2):
    ...
    lak_topics['titleAndAbstract']=lak_topics['title']+" "+lak_topics['abstract']
    list_dicts=[]
    for val in list(lak_topics['keywords'].values):
        ...
        for key,value in topics_list_ref.items():
            value=int(value)
            topics_list_ref[key]=value  
        sorted_keys_tuple = sorted(topics_list_ref.items(), reverse=True,key=operator.itemgetter(1))   
    ...
    for val in flat_list:
        for abstract,authors,title in zip(list(lak_topics['titleAndAbstract'].values),list(lak_topics['authors'].values),list(lak_topics['year'].values)):
...
def fetchTopicsuserID(ssuserid,year):
    #ssuserid=2399182
    author_data = requests.get(f"https://api.semanticscholar.org/v1/author/{ssuserid}").json()
    print(author_data)
    ...
    df_paperdata['title_abstract']=df_paperdata['title']+" "+df_paperdata['abstract']
    ...
    wikis_noquotes=wikis_noquotes.replace("}","")
    print("******************************************************************************")
    wikis_noquotes_ref = {i.split(': ')[0]: i.split(': ')[1] for i in wikis_noquotes.split(', ')}
   ...
   # list_todict = json.loads(topics_list)
    ...
    v.get_patch_by_id('11').set_alpha(0.3)
    v.get_patch_by_id('11').set_color('#3A675C')
    v.get_label_by_id('10').set_text('\n'.join(list(set(l_author)-set(l_conf))))
    label1 = v.get_label_by_id('10')          
    label1.set_fontsize(7) 
    v.get_label_by_id('01').set_text('\n'.join(list(set(l_conf)-set(l_author))))
    label2 = v.get_label_by_id('01')          
    label2.set_fontsize(7) 
    v.get_label_by_id('11').set_text('\n'.join(list_intersect_y1y2))
    label3 = v.get_label_by_id('11')          
...
def compareAuthors(author1,author2,year,key):
    ...
    if len(list(lak_query1.values))!=0:
        ...
        lak_author1_data['titleAndAbs']=lak_author1_data['title']+" "+lak_author1_data['abstract']
    ...
    if len(list(lak_query2.values))!=0:
        sorted_dict2 = dict(sorted(convertStrtoDict(lak_query2['topics'].values[0]), reverse=True,key=operator.itemgetter(1)))
        wikis_a2 = {k: sorted_dict2[k] for k in list(sorted_dict2)[0:10]}
        sorted_dict2=dict(sorted(convertStrtoDict(lak_query2['keywords'].values[0]), reverse=True,key=operator.itemgetter(1)))
        ...
        lak_author2_data['titleAndAbs']=lak_author2_data['title']+" "+lak_author2_data['abstract']
    ...
    set_intersect_wiki=list(set(wikis_a1.keys()).intersection(set(wikis_a2.keys())))
    if key=='keyword':
        authors_dict = {k:[sorted_dict1.get(k,0), sorted_dict2.get(k,0)] for k in sorted_dict1.keys() | sorted_dict2.keys()}
        keys=authors_dict.keys()
        values=authors_dict.values()
        auths=[author1,author2]
        return keys,values,auths,set_intersect_key
    elif key=='topic':
        authors_dict = {k:[wikis_a1.get(k,0), wikis_a2.get(k,0)] for k in wikis_a1.keys() | wikis_a2.keys()}
        keys=authors_dict.keys()
        values=authors_dict.values()
        auths=[author1,author2]
...
def authorConfTopicComparison(key,author,year):
    ...
    lak_author1_topics['titleandAbstract']=lak_author1_topics['title']+" "+lak_author1_topics['abstract']
    ...
    topics_list=topics_list.replace("}","")
    print("******************************************************************************")
    topics_list_ref1 = {i.split(': ')[0]: i.split(': ')[1] for i in topics_list.split(', ')}
    for key,value in topics_list_ref1.items():
        value=int(value)
        topics_list_ref1[key]=value
    sorted_dict1 = dict(sorted(topics_list_ref1.items(), reverse=True,key=operator.itemgetter(1)))
    sorted_dict1 = {k: sorted_dict1[k] for k in list(sorted_dict1)[0:10]}
    if key=='key':
    ...
    v.get_patch_by_id('11').set_alpha(0.3)
    v.get_patch_by_id('11').set_color('#3A675C')
    v.get_label_by_id('10').set_text('\n'.join(list(set(l_author_u)-set(l_conf_u))))
    label1 = v.get_label_by_id('10')          
    label1.set_fontsize(7) 
    v.get_label_by_id('01').set_text('\n'.join(list(set(l_conf_u)-set(l_author_u))))
    label2 = v.get_label_by_id('01')          
    label2.set_fontsize(7) 
    v.get_label_by_id('11').set_text('\n'.join(list_intersect_y1y2))
    label3 = v.get_label_by_id('11')          
...
def getAuthorComparisionData(author1,author2):
    ...
    print("******************************************************************************")
    topics_list_ref1 = {i.split(': ')[0]: i.split(': ')[1] for i in topics_list1.split(', ')}
    for key,value in topics_list_ref1.items():
        value=int(value)
        topics_list_ref1[key]=value
    sorted_dict1 = dict(sorted(topics_list_ref1.items(), reverse=True,key=operator.itemgetter(1)))
    ...
    print("******************************************************************************")
    topics_list_ref2 = {i.split(': ')[0]: i.split(': ')[1] for i in topics_list2.split(', ')}
    for key,value in topics_list_ref2.items():
        value=int(value)
        topics_list_ref2[key]=value
    sorted_dict2 = dict(sorted(topics_list_ref2.items(), reverse=True,key=operator.itemgetter(1)))
    sorted_dict2 = {k: sorted_dict2[k] for k in list(sorted_dict2)[0:10]}
    authors_dict = {k:[sorted_dict1.get(k,0), sorted_dict2.get(k,0)] for k in sorted_dict1.keys() | sorted_dict2.keys()}
    keys=authors_dict.keys()
    values=authors_dict.values()
