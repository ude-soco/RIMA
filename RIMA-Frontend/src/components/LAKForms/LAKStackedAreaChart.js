...
class LAKStackedAreaChart extends Component {
  ...
  clickEvent(){
    ...
    var {series}=this.state;
      ...
      fetch(BASE_URL_INTEREST+"getalltopicsevolution/"+'?'+selectedValues.join('&'))
        .then(response =>  response.json())
        .then(json => {
      ...
      fetch(BASE_URL_INTEREST+"getallkeysevolution/"+'?'+selectedValues.join('&'))
        .then(response =>  response.json())
        .then(json => {
            console.log("json",json)
            if(json!=null){
    ...
    render() {
      const checkmount=this.checkIfMounted;
        ...
        var {active2,newOptions,isLoaded,loader,active1,opacity,soptions,display,display1,selectTopic,textinput,active3,active4}=this.state;
    if(isLoaded){
      return (
  ...
  <div id="chart" className="box" >
    <Form role="form" method="POST" >
                        ...
                        <FormGroup>
                        <h2>Evolution of topics/keywords over time
                        </h2>
                        <br></br>
                        <p>This chart displays the evolution of topics/keywords over all years of the selected conference</p>
                        <br></br>
                        
                       
                        <Button color="primary" outline active={active1} value="topic" onClick={this.selectTopic}>Topic</Button>
                         <Button outline color="primary" value="key" active={active2} onClick={this.selectKey}>Keyword</Button>
       ...
       ></i> 
       {
        this.state.imageTooltipOpen && (
                            ...
                            >
<p> Hover over legend to highlight the evolution of a topic/keyword</p>
                        ...
                        <Select ref={this.selectInputRef} name="selectOptions" isClearable isMulti placeholder="Select Option" options={soptions} 
                         ...
                         onChange={this.handleChange}/>
                            ...
                            <br></br>
                        <Button outline color="primary" active={active3} onClick={ this.clickEvent } 
                        >Compare</Button>
                        <Button outline color="primary" active={active4} onClick={this.onClear}>Reset</Button>
               

                       <div style={{'marginLeft':'300px','marginTop':'100px','position': 'absolute'}}>
...
<p> Hover over legend to highlight the evolution of a topic/keyword</p>
                         ...
                         onChange={this.handleChange}/>
                            ...
                            <br></br>
                        <Button outline active={active3} color="primary"  onClick={this.clickEvent}>Compare</Button>
                        <Button outline active={active4} color="primary"  onClick={this.onClear}>Reset</Button>  
                        <div style={{'marginLeft':'300px','marginTop':'100px','position': 'absolute'}}> 
                        <div style={{'backgroundColor':'white','display':display}}>
                        <Loader type="Bars" visible={loader} color="#00BFFF" height={100} width={100}></Loader>
                        </div>
