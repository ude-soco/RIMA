...
import {ButtonGroup,Modal,ModalBody,ModalFooter,Table,Col,ModalHeader,Row} from 'reactstrap'
...
class LAKPie extends Component {
  constructor(props){
    ...
    displayAbstract(param){
      fetch(`${BASE_URL_INTEREST}`+"getabstractdetails/"+param+"/"+this.state.selectyear)
      ...
      .then(json => {  
          this.setState({
              ...
              url:'https://www.semanticscholar.org/search?year%5B0%5D='+this.state.selectyear+'&year%5B1%5D='+this.state.selectyear+'&venue%5B0%5D=LAK&q='+param+'&sort=relevance'
            ...
            chart: {
                ...
                events: {
                  dataPointSelection: function(event, chartContext, config) { 
                    ...
                    var topic=chartContext.w.config.labels[config.dataPointIndex]
  ...
  selectValue (e) {
    ...
    fetch(BASE_URL_INTEREST+"gettopicsforpie/"+this.state.selectnum+"/"+this.state.selectyear)
    .then(response =>  response.json())
    .then(json => {
  ...
  selectKey (e) {
    ...
    const displayabstract=this.displayAbstract;
    fetch(BASE_URL_INTEREST+"getkeysforpie/"+this.state.selectnum+"/"+this.state.selectyear)
    .then(response =>  response.json())
    .then(json => {
      ...
      render() {
        ...
        var {selectValue,items,weights,isLoaded,selectyear,selectnum,active1,active2,modalTitle,highlightText,url,modalBody}=this.state;
    ...
    if(isLoaded){   
          ...
          <br></br>
          <p>The pie chart displays the distribution of top 5/10 topics/keywords for the selected conference</p>
                        <Label>Select a year</Label>
                        <div style={{width: '200px'}}>
                        <Select  placeholder="Select Option" options={yeardata}  value={yeardata.find(obj => obj.value === selectyear)} 
                         ...
                         />
                          ...
                          <br></br>
                          <div style={{width: '200px'}}>
                          <Select  placeholder="Select Option" options={numbers}  value={numbers.find(obj => obj.value === selectnum)} 
                         onChange={this.selectNumber}
                         />
                         </div>
                       <br></br>
                     
                         <Button outline color="primary" active={active1} onClick={this.selectValue}>Topic</Button>
                         <Button outline color="primary" active={active2} onClick={this.selectKey}>Keyword</Button>
       ...
       onMouseOut={() => this.handleToogle(false)}
        ...
        ></i> 
         {this.state.imageTooltipOpen && (
                            ...
                            >
                            <p>Click on a sector of pie chart to view more details or double click the legend</p>
