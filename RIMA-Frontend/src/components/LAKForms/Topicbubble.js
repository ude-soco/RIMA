...
import {Label,Modal,ModalHeader,ModalBody,ModalFooter,Button,Table,PopoverHeader,PopoverBody} from 'reactstrap';
...
class Topicbubble extends Component {
 ...
 onClickNode(nodeId) {
   // window.alert(`Clicked node ${nodeId}`);
    ...
    fetch("http://127.0.0.1:8000/api/interests/getabstractdetails/"+nodeId+"/"+this.state.selectYear)
...
selectValue(e){
    if(e.value=='topic'){
        fetch("http://127.0.0.1:8000/api/interests/getoverviewtopicdetails/"+e.value+"/"+this.state.selectYear)
        .then(response =>  response.json())
        .then(json => {
    ...
    if(e.value=='keyword'){
        fetch("http://127.0.0.1:8000/api/interests/getoverviewkeydetails/"+e.value+"/"+this.state.selectYear)
        .then(response =>  response.json())
        .then(json => {
  ...
  render() {
    var {graphData,selectVal,isLoaded,modalBody,modalTitle,myConfig,selectYear,highlightText}=this.state;
