...
class PreloadedConferenceList(models.Model):
    conference_name_abbr = models.CharField(
...
class Conference_Event(models.Model):
    conference_name_abbr = models.ForeignKey(Conference,
                                             ...
                                             on_delete=models.CASCADE)
    conference_event_name_abbr = models.CharField(
        max_length=100, primary_key=True)
    conference_event_full_name = models.CharField(
        max_length=255, null=True, blank=True)
    conference_event_url = models.CharField(
...
class Author(models.Model):
    semantic_scolar_author_id = models.CharField(
    ...
    papers_within_conference = models.ManyToManyField(
        Conference_Event_Paper, through='Author_has_Papers')
...
class Author_has_Papers(models.Model):
    ...
    conference_event_name_abbr = models.ForeignKey(
        Conference_Event, on_delete=CASCADE)

    class Meta:
        unique_together = (
            ("conference_event_name_abbr", "author_id", "paper_id"),)
...
class Conf_Event_keyword(models.Model):
    ...
    conference_event_name_abbr = models.ManyToManyField(
        Conference_Event, through='Event_has_keyword')


class Event_has_keyword(models.Model):
    conference_event_name_abbr = models.ForeignKey(
        Conference_Event, on_delete=CASCADE, related_name='keywords_Events')
    keyword_id = models.ForeignKey(
        Conf_Event_keyword, on_delete=CASCADE, related_name='haskeywords')
...
class Conf_Event_Topic(models.Model):
    ...
    algorithm = models.CharField(max_length=255, null=True, blank=True)
    conference_event_name_abbr = models.ManyToManyField(
        Conference_Event, through='Event_has_Topic')


class Event_has_Topic(models.Model):
    conference_event_name_abbr = models.ForeignKey(
        Conference_Event, on_delete=CASCADE, related_name='Topics_Events')
    topic_id = models.ForeignKey(
        Conf_Event_Topic, on_delete=CASCADE, related_name='hastopics')
    weight = models.IntegerField(null=True)
...
class Author_Event_Topic(models.Model):
    ...
    author_id = models.ManyToManyField(
        Conference_Event, through='Author_has_Topic')


class Author_has_Topic(models.Model):
    author_id = models.ForeignKey(
        Author, on_delete=CASCADE, related_name='authors_topics')
    conference_event_name_abbr = models.ForeignKey(
        Conference_Event, on_delete=CASCADE)
    topic_id = models.ForeignKey(
        Author_Event_Topic, on_delete=CASCADE, related_name='authorhastopics')
    weight = models.IntegerField(null=True)

    class Meta:
        unique_together = (
            ("author_id", "topic_id", "conference_event_name_abbr"),)
...
class Author_Event_keyword(models.Model):
    ...
    algorithm = models.CharField(max_length=255, null=True, blank=True)
    author_id = models.ManyToManyField(
        Conference_Event, through='Author_has_Keyword')


class Author_has_Keyword(models.Model):
    author_id = models.ForeignKey(
        Author, on_delete=CASCADE, related_name='authors_keywords')
    conference_event_name_abbr = models.ForeignKey(
        Conference_Event, on_delete=CASCADE, null=True, blank=True)
    keyword_id = models.ForeignKey(
        Author_Event_keyword, on_delete=CASCADE, related_name='authorhaskeywords')
    weight = models.IntegerField(null=True)
    all_events = models.BooleanField(default=False)

    class Meta:
        unique_together = (
            ("author_id", "keyword_id", "conference_event_name_abbr"),)
