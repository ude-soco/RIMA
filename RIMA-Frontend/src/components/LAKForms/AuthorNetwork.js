...
import {Label,Modal,ModalHeader,ModalBody,Button,ModalFooter,InputGroup,InputGroupAddon,Input,Row,Col,Form,FormGroup} from 'reactstrap';
...
handleKeyDown(e) {
 ...
 onClickNode(nodeId) {
   // window.alert(`Clicked node ${nodeId}`);
    ...
    fetch(BASE_URL_INTEREST+"getallauthorslist/"+nodeId+"/"+this.state.nodeval
...
selectValue(e){
    ...
    fetch(BASE_URL_INTEREST+"getalltitles/"+e.value+"/"+this.state.selectYear)
    .then(response =>  response.json())
    .then(json => {
     this.state.graphData=[];   
        this.setState({
...
handleToogle = (status) => {
  ...
  render() {
    ...
    var {keywords,authors,nodeval,graphData,selectVal,titles,isLoaded,count,
      isModalLoader,myConfig,selectYear,url,papers,noder,active1,active2,authorids}=this.state;
    ...
    return (
      ...
      'borderRadius':'2px'}}>
        ...
        <h3>Researcher collaboration network</h3>
               <br></br>
               <p>This visualization displays the collaboration of researchers for the selected year related to a specific topic/keyword</p>
        <Form>
          <FormGroup>
        <br></br>
       ...
       <Col>
                          ...
                          <Button color="primary" onClick={this.setZoomIn}> <i  className="fas fa-search-plus text-white"></i></Button>
                          <Button color="primary" onClick={this.setZoomOut}> <i  className="fas fa-search-minus text-white"></i></Button>
                          <Button color="primary" onClick={this.cancelZoom}> <i  className="fas fa-home text-white"></i></Button>
                         ...
                         <InputGroup>
        <Input id='searchkey' placeholder="search researcher in graph" />
        <InputGroupAddon addonType="append">
          <Button color="primary" onClick={this.findText} onKeyDown={this.handleKeyDown}><i  className="fas fa-search text-white"></i></Button>
        </InputGroupAddon>
      </InputGroup>
      </div>
                         </Col>
...
<div style={{'backgroundColor':'#F7F7F7','padding':'20px','border':'0.5px solid #F7F7F7'}}>  
    <ModalHeader toggle={this.toggle}><h3> Details related to '{noder}'</h3></ModalHeader>
    </div>
        <ModalBody>
  ...
  <Badge color="info">{count}</Badge> 
    ...
    <b>{count} Publication(s) related to the topic '{nodeval}'</b>
    <br></br>
    <br></br>
      {papers.map((item,index) => {
          ...
          return <li style={{'whiteSpace': 'unset'}}><a href={item} title="click to view paper in semantic scholar" target="_blank">{title}</a><br></br>
          <p><b>Researchers:</b>
          {alist.map((item,index) => {
          const aiditem=idlist[index];
          return <a title="click to view researcher profile in semantic scholar" href={"https://www.semanticscholar.org/author/"+aiditem} target="_blank"
          >{item},</a>})}</p>
          </li>;
        })}
        </div>
        ...
        <ModalFooter>
          <Col>    
          <Button  color="info"><a href={url} target="_blank" style={{'color':'white'}}>Researcher Profile in semantic scholar</a></Button>
</Col>
          <Col></Col>
          <Col></Col>
          <Col></Col>
          <Col>
          <Button onClick={this.toggle} >Close</Button></Col>{' '}
