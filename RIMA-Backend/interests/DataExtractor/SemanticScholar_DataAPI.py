...
extracted = [string.replace("https://doi.org/", "") for string in flatList_dois]


# In[270]:


extracted_dois = [string.replace("http://dl.acm.org/citation.cfm?id=", "10.1145/3027385.") for string in extracted]
...
for val in list_paperdata[616:]:
    if 'doi' in val:
        ...
        try:
            ...
            cursor=conn.cursor()
            postgreSQL_insert_Query = "insert into LAKData(id,title,abstract,venue,year,authors,authorids,dois) values('"+str(paperid)+"','"+str(title)+"','"+str(abstract)+"','"+str(venue)+"','"+str(year)+"','"+str(authorNames)+"','"+str(authorIds)+"','"+str(doi)+"')"
