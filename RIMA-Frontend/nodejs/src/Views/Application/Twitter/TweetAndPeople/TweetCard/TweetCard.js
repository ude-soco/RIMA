...
`;
//Changed to functional component and switching to MUI by Yasmin
export function TweetContent(props) {
  return <span dangerouslySetInnerHTML={{ __html: props.modified_text }} />;
}
...
export default function TweetCard(props) {
  ...
  const calculateSimilarity = async () => {
    let { tweet, keyword_tags } = props;
    keyword_tags = keyword_tags.filter((i) => i.text == props.tweet.text);
    const data = {
      text: tweet["full_text"].trim(),
      algorithm: "Yake",
    };
    const keywordArray = [];
    try {
    ...
    if (keywordArray.length !== 0) {
      for (const userInterest of keyword_tags) {
        let data = [];
  ...
  const saveHandler = (params) => {
    ...
    RestAPI.savedTweets(data)
      .then((response) => {
        props.newSavedTweet(response.data);
        ...
        <>
          ...
          <div className="card-body">
            ...
            <p className="card-text">
              ...
              >
                <TweetContent modified_text={state.tweetModiText} />
              </a>
