...
class NewSilmilarityEvolution extends Component {
  ...
  compareSharedTotalAuthors = () => {
    console.log("compareSharedTotalAuthors called")
    ...
    fetch(
      ...
      .then((json) => {
        ...
        series = [];
        weights = [];
        let totalNumberOfAuthors = [];
        let numberOfSharedAuthors = [];
        let similaritOfAuthorsIndex = [];
        let firstYears = [];
        ...
        if (this.state.selectedConferences.length == 1) {
          for (let index = 0; index < json.years.length; index++) {
            ...
            console.log("similaritOfAuthorsIndex100 :  ",similaritOfAuthorsIndex)
          }
          ...
          for (let i = 0; i < json.weights.length; i++) {
            ...
            for (let index in similaritOfAuthorsIndex) {
              console.log("type od: ",typeof (similaritOfAuthorsIndex[index]));
              if (typeof (similaritOfAuthorsIndex[index]) === "NaN") {
                similaritOfAuthorsIndex[index] = 0;
              }
            }
            console.log(
              " similaritOfAuthorsIndex ,",i," ",
    ...
    fetch(
      ...
      .then((json) => {
        ...
        console.log(
         ...
         (isNaN(finalSimilarityIndex[2]))

        );
        for (let index in finalSimilarityIndex) {
          if (isNaN(finalSimilarityIndex[index])) {
            finalSimilarityIndex[index] = 0;
         }
        }
              ...
              <Col>
                ...
                {this.state.imageTooltipOpen && (
                  ...
                  >
                    <p>
                      {" "}
                      This similarity index is made from the calculation of the
                      percentage of the shared author to the number of total
                      authors plus the number of shared topics/keywords between
                      selected conferences over their shared years.{" "}
                    </p>
                  </div>
                )}
              </Col>
