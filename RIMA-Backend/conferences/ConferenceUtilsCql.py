...
def CreateDatabaseLabels(tx):
    cqlConfernceLabel = "CREATE CONSTRAINT ON (c:Conference) ASSERT c.conference_name_abbr IS UNIQUE"
    cqlEventLabel = "CREATE CONSTRAINT ON (e:Event) ASSERT e.conference_event_name_abbr IS UNIQUE"
    cqlPublicationLabel = "CREATE CONSTRAINT ON (p:Publication) ASSERT p.paper_id IS UNIQUE"
    cqlAuthorLabel = "CREATE CONSTRAINT ON (a:Author) ASSERT a.semantic_scolar_author_id IS UNIQUE"
    cqlKeywordLabel = "CREATE CONSTRAINT ON (k:Keyword) ASSERT k.keyword IS UNIQUE"
    cqlTopicLabel = "CREATE CONSTRAINT ON (t:Topic) ASSERT t.topic IS UNIQUE"
    # add here any new cql label, then add it to queries list
    queries = [cqlConfernceLabel, cqlEventLabel, cqlPublicationLabel,
               cqlAuthorLabel, cqlKeywordLabel, cqlTopicLabel]
...
def CreateEvent(tx, name_abbr, url):
...
def GetAuthorKeywordWeight(tx, authorID, keyword):
...
def GetAuthorTopicWeight(tx, authorID, topic):
...
def CheckEventTopicRelation(tx, event, topic):
    result = tx.run(
...
def CheckEventKeywordRelation(tx, event, keyword):
    result = tx.run(
...
def CheckAuthorKeywordRelation(tx, authorID, keyword):
    result = tx.run(
...
def CheckAuthorTopicRelation(tx, authorID, topic):
    result = tx.run(
...
def GetPublicationFromKeyword(tx, event, keyword):
    result = tx.run(
...
def GetPublicationFromTopic(tx, event, topic):
    result = tx.run(
...
def GetAuthorKeyword(tx, authorID):
...
def GetAuthorTopic(tx, authorID):
...
def GetAuthorByName(tx, author_name):
    result = tx.run(
...
def UpdateAuthorKeywordRelation(tx, authorID, keyword, weight):
    tx.run(
...
def UpdateAuthorTopicRelation(tx, authorID, topic, weight):
    tx.run(
...
def UpdateEventTopicRelation(tx, event, topic, weight):
    tx.run(
